package me.majiajie.codec;


public class CustomBase64_Decode extends Custom
{

    protected final char[] DEFAULT_ALPHABET = {
        //	A~Z
        0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48,0x49,0x4A,0x4B,0x4C,0x4D,0x4E,0x4F,
        0x50,0x51,0x52,0x53,0x54,0x55,0x56,0x57,0x58,0x59,0x5A,
        //	a~z
        0x61,0x62,0x63,0x64,0x65,0x66,0x67,0x68,0x69,0x6A,0x6B,0x6C,0x6D,0x6E,0x6F,
        0x70,0x71,0x72,0x73,0x74,0x75,0x76,0x77,0x78,0x79,0x7A,
        //	0~9
        0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,
        //	+ /
        0x2B,0x2F
        };


    protected String mString;

    protected char[] mAlphabet;

    protected char mFillChar;

    protected int mMaxCharLine;

    public CustomBase64_Decode(String string)
    {
        mString = string;
        mAlphabet = DEFAULT_ALPHABET;
    }

    /**
     * 设置字母表。<br/>
     * @param alphabet 64位的字符数组
     * @return
     */
    public CustomBase64_Decode setAlphabet(char[] alphabet)
    {
        mAlphabet = alphabet;
        return this;
    }

    /**
     * 设置末尾填充的字符，默认是‘=’。
     * 填充字符是在编码字符位数不足时使用。<br/>
     * <strong>注意：</strong>解码和编码需要设置相同的
     */
    public CustomBase64_Decode setFillChar(char fillChar) {
        mFillChar = fillChar;
        return this;
    }

    /**
     * 设置每行最大的字符数。即加入换行符'\n'。<br/>
     * 这里我写的算法默认是不换行的，但按BASE64标准的话是需要每76个字符加一个换行符的<br/>
     * <strong>注意：</strong>解码和编码需要设置为相同的
     */
    public CustomBase64_Decode setMaxCharLine(int n)
    {
        mMaxCharLine = n;
        return this;
    }

    @Override
    public String doit()
    {
        return NativeMethod.Base64Decode(mString, mAlphabet, mFillChar, mMaxCharLine);
    }
}
